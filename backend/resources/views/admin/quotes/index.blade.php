...
@section('content')
<h1 class="text-2xl font-semibold mb-6">Cotizaciones solicitadas</h1>
        ...
        <tbody class="divide-y divide-gray-200">
            @forelse($submissions as $s)
                <tr>
                    ...
                    <td class="px-4 py-3 text-sm max-w-xs truncate">{{ $s->message ? Str::limit($s->message, 50) : '—' }}</td>
                    <td class="px-4 py-3 text-sm text-gray-500">{{ $s->created_at->format('d/m/Y H:i') }}</td>
                </tr>
            @empty
                <tr><td colspan="5" class="px-4 py-8 text-center text-gray-500">No hay cotizaciones.</td></tr>
            @endforelse
        </tbody>
    </table>
